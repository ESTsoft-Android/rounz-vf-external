...
interface FaceDetectingListener {
    /**
     * 얼굴이 인식된 여부를 알려주는 콜백입니다.
     *
     * 얼굴영역이 가이드라인 안으로 들어오더라도 [onFaceDetected] 콜백이 불린 상태면,
     * [FifteenCutProfile.unbind] 호출되기 전까지
     * [isDetected] 값이 `true` 값으로 절대 넘어오지 않습니다.
     *
     * @param isDetected 얼굴인식 여부
     */
    fun onFaceDetecting(isDetected: Boolean)
